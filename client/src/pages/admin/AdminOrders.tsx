    ...
    <div className="space-y-8 text-start">
      ...
      <div className="grid gap-6 text-start">
        {filteredOrders.map((order: any) => {
          ...
          let displayPhoto = '';
          if (firstItem) {
              if (firstItem.product?.image) displayPhoto = firstItem.product.image;
              else if (firstItem.customerPhoto) {
                  try {
                      if (firstItem.customerPhoto.startsWith('[')) {
                          const photos = JSON.parse(firstItem.customerPhoto);
                          displayPhoto = photos[0];
                      } else {
                          displayPhoto = firstItem.customerPhoto;
                      }
                  } catch (e) { displayPhoto = firstItem.customerPhoto; }
              }
          }
